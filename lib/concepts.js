...
class Concepts{
  ...
  save(filename){
    var fs = require('fs');
    var content = JSON.stringify(this.products,0,2);
    ...
    return Promise.mapSeries(products,function(product,i){
      ...
      predicts.forEach(function(concepts,i){
        this.products.push({
          ...
          tags:concepts.filter(function(concept) {return (concept.value>0.95);}).map(function(e){return e.name}),
          updated:today
        });
  ...
  buildIndex(products){
    ...
    var prod_sz=products.length;

    //
    // already done
    if(typeof products[1].tags[0]==='string'){      
      //
      // compute tags freq
    ...
    products.forEach(prod =>{
      ...
      if(!prod.tags.length||prod.tags[0].score<1.0)return;
      if(!this.graph.best[prod.tags[0].name]){
        this.graph.best[prod.tags[0].name]=[];
      }
      this.graph.best[prod.tags[0].name].push({
        product:prod,
        score:prod.tags[0].score
      }); 
    });
  ...
  similar(product){
    ...
    return this.graph.best[product.tags[0].name];
  }
